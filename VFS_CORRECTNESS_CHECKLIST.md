...
| 契約 | 主な確認場所 | 状態 |
| --- | --- | --- |
| create / update / revive | `fs_store_basic`, `fs_store_scale`, `wiki_canister` | 検証済み |
| `etag` 競合 | `fs_store_basic`, `fs_store_vfs`, `wiki_canister` | 検証済み |
| tombstone / revive | `fs_store_basic`, `fs_store_sync`, `tests_sync_contract` | 検証済み |
...
| `search_nodes` prefix / tombstone 非表示 | `fs_store_basic`, `fs_store_scale`, `tests_sync_contract` | 検証済み |
...
| `fetch_updates` deleted visibility | `fs_store_sync`, `tests_sync_contract` | 検証済み |
...
- prefix 制限つき `search_nodes` と tombstone 非表示
- 大きい snapshot に対する小差分 `fetch_updates`
- canister 境界での deleted visibility / prefix scope change
