...
use vfs_types::{ListNodesRequest, Node, NodeEntry, NodeEntryKind, NodeKind, WriteNodeRequest};
use wiki_domain::{
    WIKI_CONCEPTS_PREFIX, WIKI_ENTITIES_PREFIX, WIKI_INDEX_PATH, WIKI_ROOT_PATH,
    WIKI_SOURCES_PREFIX,
};
...
pub async fn rebuild_index(client: &impl VfsApi) -> Result<()> {
    ...
    for entry in entries {
        ...
        if entry.path.starts_with(&format!("{WIKI_SOURCES_PREFIX}/")) {
            sources.push(rendered);
        } else if entry.path.starts_with(&format!("{WIKI_ENTITIES_PREFIX}/")) {
            entities.push(rendered);
        } else if entry.path.starts_with(&format!("{WIKI_CONCEPTS_PREFIX}/")) {
...
pub async fn rebuild_scope_index(client: &impl VfsApi, scope: &str) -> Result<()> {
    ...
    for current_scope in &ancestors {
        let scope_prefix = format!("{WIKI_ROOT_PATH}/{current_scope}");
...
fn render_root_scope_item(node: &Node) -> Option<String> {
    let relative = node.path.strip_prefix(&format!("{WIKI_ROOT_PATH}/"))?;
...
fn normalize_scope_name(scope: &str) -> Result<String> {
    ...
    let wiki_root_name = WIKI_ROOT_PATH.trim_start_matches('/');
    let without_prefix = trimmed
        .strip_prefix(&format!("{wiki_root_name}/"))
        .or_else(|| trimmed.strip_prefix(wiki_root_name))
        .unwrap_or(trimmed);
