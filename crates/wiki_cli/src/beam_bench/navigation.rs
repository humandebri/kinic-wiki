...
use anyhow::Result;
use wiki_types::{ListNodesRequest, NodeEntryKind, NodeKind, WriteNodeRequest};
...
pub async fn sync_beam_indexes(client: &impl WikiApi, namespace: &str) -> Result<()> {
    ...
    let root_index = upsert_list_entry(&current_root, "Benchmarks", &beam_entry, &index_path);
    upsert_node(client, ROOT_INDEX_PATH, &root_index).await?;
    Ok(())
}

async fn upsert_node(client: &impl WikiApi, path: &str, content: &str) -> Result<()> {
...
mod tests {
    ...
    impl WikiApi for MockClient {
        async fn status(&self) -> Result<wiki_types::Status> {
            unreachable!()
        }
        ...
        ) -> Result<Vec<wiki_types::SearchNodeHit>> {
            unreachable!()
        }
