...
export default function DashboardDatabasePage() {
  ...
  const databaseId = useMemo(() => databaseIdFromPathname(pathname), [pathname]);
    ...
    async (client: AuthClient, nextDatabaseId: string) => {
      ...
      try {
        ...
        if (nextDatabase?.role === "owner") {
          try {
            setMembers(await listDatabaseMembersAuthenticated(canisterId, identity, nextDatabaseId));
          } catch (cause) {
...
function databaseIdFromPathname(pathname: string): string {
  return decodeURIComponent(pathname.split("/").filter(Boolean)[1] ?? "");
}
