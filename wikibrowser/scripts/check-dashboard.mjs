...
assert.match(wranglerConfig, /"REPLACE_WITH_QUERY_ANSWER_RATE_LIMIT_KV_NAMESPACE_ID"/);
...
assert.match(ingestPanel, /TriggerSessionState = "checking" \| "ready" \| "denied"/);
assert.match(ingestPanel, /ensureUrlIngestTriggerSession\(canisterId, databaseId, readIdentity\)/);
assert.match(ingestPanel, /triggerSessionState === "denied" \? "URL ingest disabled"/);
